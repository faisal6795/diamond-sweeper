		...
		if (!this.state.pressed)
			return (
				<button
					...
					onClick={() => {
						document.getElementById(`button${this.props.row}${this.props.col}`)
							.className = 'button-rotate';
						this.props.decrementCounter();
						let isDiamond = false;
						let nearestNeighbour;
						if (this.props.diamondPositions
							.findIndex(pos => pos.r === this.props.row && pos.c === this.props.col) !== -1) {
							isDiamond = true;
						} else {
							let minR = SIZE;
							let minC = SIZE;
							this.props.diamondPositions.forEach(diamond => {
								if ((Math.abs(diamond.r - this.props.row) +
									Math.abs(diamond.c - this.props.col)) / 2 < (minC + minR) / 2) {
									minR = Math.abs(diamond.r - this.props.row);
									minC = Math.abs(diamond.c - this.props.col);
									nearestNeighbour = diamond;
								}
							});
							if (Math.abs(nearestNeighbour.r - this.props.row) <
						...
						setTimeout(() => this.setState({ isDiamond }, () => {
							this.setState({ pressed: true });
							document.getElementById(`button${this.props.row}${this.props.col}`).className = 'button';
						}),
							700
						);
					}}
